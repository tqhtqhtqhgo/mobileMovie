    ...
    export default {
        ...
        methods:{
            ...
            if(this.$route.query.movie_id){
              ...
              if (json.success_code===200){
                ...
                if(localStorage.getItem('user_id')){
                  //判断用户是否喜欢该电影
                  let json = await isWishMovie(localStorage.getItem('user_id'),this.$route.query.movie_id);
                  if (json.success_code===200){
          ...
          async wishBtnHandle(){
            if (localStorage.getItem('user_id')){
              ...
              if (this.notWishMovie){
                let json = await wishMovie(localStorage.getItem('user_id'),this.$route.query.movie_id);
                if (json.success_code===200){
                  this.notWishMovie = false;
                  this.loadMovieDetail();
                }
              } else {
                let json = await cancelWishMovie(localStorage.getItem('user_id'),this.$route.query.movie_id);
                if (json.success_code===200){
                  this.notWishMovie = true;
                  this.loadMovieDetail();
                }
            ...
            if (localStorage.getItem('user_id')){
              ...
              if (json.success_code===200){
                ...
                if (json.data.support_user) {
                  ...
                  //当前用户已点赞
                  if (supportUser.indexOf(Number(localStorage.getItem('user_id')))>-1) {
                    //取消点赞
                    supportUser.splice(supportUser.indexOf(Number(localStorage.getItem('user_id'))),1);
                    ...
                    //点赞
                    supportUser.push(Number(localStorage.getItem('user_id')));
                    supportNum = supportUser.length;
                  }
                } else {
                  //无点赞数据
          ...
          userIsSupportComment(supportStrArr){
            if (supportStrArr&&JSON.parse(supportStrArr).indexOf(Number(localStorage.getItem('user_id')))>-1) {
              return true;
            } else {
              return false;
            }
