...
<!--          <span class="date"><span class="calender"><span class="day">{{new Date().getDate()<10?'0'+new Date().getDate():new Date().getDate()}}</span></span></span>-->
        ...
        <div class="swiper-container">
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide">
              <img src="./images/swiper/2.jpg" alt="" @click="$router.push({path:'/movie_detail',query:{movie_id:17}})">
            </div>
            <div class="swiper-slide">
              <img src="./images/swiper/3.jpg" alt="" @click="$router.push({path:'/movie_detail',query:{movie_id:33}})">
            </div>
            <div class="swiper-slide">
              <img src="./images/swiper/4.jpg" alt="" @click="$router.push({path:'/movie_detail',query:{movie_id:55}})">
            </div>
          </div>
          <!-- 如果需要分页器 -->
...
<script>
import {getCinemaList, getMovieList, getWishMovieByUserId} from '../../api/index'
    ...
    export default {
        ...
        created(){
          ...
          this.loadMovieList();
          this.loadWishMovieList();
          this.loadCinemaListList();
      ...
      methods:{
        ...
        async loadMovieList(){
          ...
          this.hotMovieList.slice(0,5);
          this.notShowMovieList.sort((a,b)=>{
            return b.wish_num-a.wish_num;
          });
          this.notShowMovieList.slice(0,3);
          Indicator.close();
        },
        async loadWishMovieList(){
          let json = await getWishMovieByUserId(55);
          json.data.forEach((value,index)=>{
            if(index<3){
              this.IwishMovie.push(value);
            }
          });
          this.IwishMovie.sort((a,b)=>{
            return b.wish_num-a.wish_num;
          });

          Indicator.close();
        },
        async loadCinemaListList(){
          let json = await getCinemaList();
          json.data.forEach((value,index)=>{
              this.CinemaList.push(value);
          });
          this.CinemaList.slice(0,2);
          this.CinemaList[0].province="离我最近"
          this.CinemaList[1].province="离我次近"
          Indicator.close();
        }
      }
    }
