...
<script>
    ...
    export default {
        ...
        data(){
          return{
            jsonData:{},
            avatar:'http://47.93.8.38:3002/images/avatar/userIcon.png'
          }
        },
        created(){
          this.loadUserInfo();
        ...
        methods:{
          ...
          userAvatar(){
            if (this.jsonData){
              this.avatar = 'http://47.93.8.38:3002'+this.jsonData.avatar
            } else {
              this.avatar = 'http://47.93.8.38:3002/images/avatar/userIcon.png'
            }
          },
          //加载用户信息
          async loadUserInfo(){
          ...
          viewMyOrder(flag){
            if (localStorage.getItem('user_id')) {
              this.$router.push({path:'my_order',query:{'user_id':localStorage.getItem('user_id')}});
            } else{
          ...
          viewMyMovie(flag){
            if (localStorage.getItem('user_id')) {
              this.$router.push({path:'my_movie',query:{'user_id':localStorage.getItem('user_id'),'wish_movie':flag}});
            } else{
              this.$router.push('login');
            }
          },
