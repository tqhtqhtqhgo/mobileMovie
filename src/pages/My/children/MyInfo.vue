  ...
  methods: {
    ...
    updateUserInfo() {
      if (this.jsonData) {
        ...
        this.avatar = "http://47.93.8.38:3002" + this.avatarSrc;
    ...
    async modifyUserSex(sex) {
      if (localStorage.getItem("user_id") && sex) {
        let json = await updateUserSex(localStorage.getItem('user_id'), sex);
    ...
    async modifyUserPwd(pwd) {
      if (localStorage.getItem("user_id") ) {
        let json = await updateUserPwd(localStorage.getItem('user_id'), pwd);
        if (json.success_code == 200) {
    ...
    async loadUserInfo() {
      if (localStorage.getItem('user_id')) {
        let json = await getUserInfo(localStorage.getItem('user_id'));
        if (json.success_code === 200) {
    ...
    async handleConfirm(selectedDate) {
      if (localStorage.getItem('user_id') && selectedDate) {
    ...
    //修改密码
    changeUserPwd() {
      MessageBox.prompt("修改密码(6-12位)").then(({ value, action }) => {
        if (value && /^[\w_-]{6,12}$/.test(value)) {
          this.modifyUserPwd(value);
        } else {
          MessageBox.alert("请输入6到12位密码,且为字母及数字！");
        }
      }).catch(e=>e);
    },
    //退出登录
